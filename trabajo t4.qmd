...
hist(x1,freq=FALSE, border="green",main='Estimadores de la función de densidad', xlab='', ylim=c(0,0.03)) 
lines(d1_x1, col='blue', lwd= 2 )
lines(d2_x1, col = 'red',lwd= 2)
legend("topright", legend = c("LSCV", "Plug-in"), col = c("blue","red"),lwd = 2)
```

En este otro observamos diferencias entre las aproximaciones. La estimacion del LSCV es mas rugosa, tiene más pliegues, y en cambio la del plug in es muy suave. Esto se debe a que la venta del plug in es notablemente mayor que la del LCSV. Es distinto al ejemplo anterior, porque en el anterior ejercico las ventanas eran muy parecidas. 
...
Plug in es una estimacion más suave de nuevo que la estimacion CV. En este probablemente sería una idea usar el método plug in porque parece adaptarse mejor a la varianza de los datos. 
