...
author: "Miguel García Moreno <br/> Hugo Montilla Anta <br/> Lola Velasco Ballesta"
date: today
lang: es
...
Para la función de densidad conjunta usaremos la librería `ks`, tiene establecido el núcleo gaussiano por defecto. 
...
Expresamos la función de densidad conjunta usando `kde`, si no se especifica la matriz de ventanas usa el método plug-in, aun así se lo indicamos. Representamos los contornos.

```{r}
fhat.pi = kde(mat,H=Hp) 

v= 1:9*10
plot(fhat.pi, cont=v, lwd=3) # añadimos contornos
```

Comparamos con la distribución.

```{r}
points(x1,y_masc, pch=20, cex=0.5)
```
...
lines(m_vc, col="darkgreen", lwd=2)
```
...
Recta de regresión
```{r}
predict(lr, newdata = data.frame(x1 = 110))
```

Estimador local lineal con validación cruzada
```{r}
approx(m_pi$x, m_pi$y, xout = 110)$y
```

Estimador local lineal con plug-in
```{r}
approx(m_vc$x, m_vc$y, xout = 110)$y
```
...
$y_{masc} = 46.317364 - 0.007677 x_2 $
...
Recta de regresión
```{r}
predict(lr, newdata = data.frame(x2 = 3000))
```

Estimador local lineal con validación cruzada
```{r}
approx(m_pi$x, m_pi$y, xout = 3000)$y
```

Estimador local lineal con plug-in
```{r}
approx(m_vc$x, m_vc$y, xout = 3000)$y
